...
export const getClassScheduleService = async (page: number, limit: number, trainerId: string) => {
    ...
    const bookings = await ClassSchedule.find(query)
        .populate('trainer', 'fullName email')
        .skip(skip)
        .limit(limit);
    const totalBookings = await ClassSchedule.countDocuments(query);
